(function (root, factory) {
  ...
  function normalizeTask(input = {}) {
    ...
    return { id: input.id || makeId(), title, time: String(input.time ?? '').trim(), priority, category, notes: String(input.notes ?? '').trim(), completed: Boolean(input.completed), createdAt };
  }
  ...
  function loadTasks(storageValue) {
    ...
    try { const parsed = JSON.parse(storageValue); return Array.isArray(parsed) ? parsed.map(normalizeTask).filter(Boolean) : []; }
    catch { return []; }
  }
  ...
  function removeTask(tasks = [], id) { return tasks.some((item) => item.id === id) ? tasks.filter((item) => item.id !== id) : tasks; }
  function mapAiTaskToPlannerTask(aiTask = {}) {
    return normalizeTask({ title: aiTask.title, time: aiTask.time, category: aiTask.category, priority: AI_TO_PLANNER_PRIORITY[aiTask.priority] || 'medium', notes: aiTask.notes, completed: false });
  }
  function findTimeConflicts(tasks = []) {
    ...
    for (const indexes of byTime.values()) if (indexes.length > 1) indexes.forEach((index) => conflicts.add(index));
    return conflicts;
  }
  return { normalizeTask, calculateProgress, sortTasks, loadTasks, upsertTask, removeTask, mapAiTaskToPlannerTask, findTimeConflicts };
});
